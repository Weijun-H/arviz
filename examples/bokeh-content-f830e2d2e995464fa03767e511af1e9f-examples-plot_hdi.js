(function() {
  const fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      const force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      const element = document.getElementById("a2506096-24f6-4a32-9ed0-f46522568a89");
        if (element == null) {
          console.warn("Bokeh: autoload.js configured with elementid 'a2506096-24f6-4a32-9ed0-f46522568a89' but no matching script tag was found.")
        }
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error(url) {
          console.error("failed to load " + url);
        }
    
        for (let i = 0; i < css_urls.length; i++) {
          const url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        for (let i = 0; i < js_urls.length; i++) {
          const url = js_urls[i];
          const element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.async = false;
          element.src = url;
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      
      const js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-2.4.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-gl-2.4.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.4.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.4.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-mathjax-2.4.1.min.js"];
      const css_urls = [];
      
    
      const inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          (function() {
            const fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  const docs_json = '{"7a0d7b42-5d9c-4cc5-823c-3f7ba11966e6":{"defs":[],"roots":{"references":[{"attributes":{"coordinates":null,"group":null},"id":"22172","type":"Title"},{"attributes":{"fill_alpha":0.2,"fill_color":"#ff0000","hatch_alpha":0.2,"line_alpha":0.2,"line_color":"#ff0000","x":{"field":"x"},"y":{"field":"y"}},"id":"22163","type":"Patch"},{"attributes":{},"id":"22142","type":"PanTool"},{"attributes":{"coordinates":null,"fill_alpha":0.5,"fill_color":"lightgrey","group":null,"level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"syncable":false,"xs_units":"screen","ys_units":"screen"},"id":"22150","type":"PolyAnnotation"},{"attributes":{"source":{"id":"22166"}},"id":"22171","type":"CDSView"},{"attributes":{},"id":"22125","type":"DataRange1d"},{"attributes":{},"id":"22141","type":"ResetTool"},{"attributes":{"line_alpha":0.1,"line_width":3,"x":{"field":"x"},"y":{"field":"y"}},"id":"22168","type":"Line"},{"attributes":{},"id":"22127","type":"DataRange1d"},{"attributes":{},"id":"22179","type":"AllLabels"},{"attributes":{"coordinates":null,"data_source":{"id":"22166"},"glyph":{"id":"22167"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"22169"},"nonselection_glyph":{"id":"22168"},"view":{"id":"22171"}},"id":"22170","type":"GlyphRenderer"},{"attributes":{},"id":"22183","type":"Selection"},{"attributes":{},"id":"22181","type":"Selection"},{"attributes":{},"id":"22129","type":"LinearScale"},{"attributes":{"data":{"x":{"__ndarray__":"","dtype":"float64","order":"little","shape":[400]},"y":{"__ndarray__":"","dtype":"float64","order":"little","shape":[400]}},"selected":{"id":"22181"},"selection_policy":{"id":"22180"}},"id":"22160","type":"ColumnDataSource"},{"attributes":{"line_alpha":0.2,"line_width":3,"x":{"field":"x"},"y":{"field":"y"}},"id":"22169","type":"Line"},{"attributes":{},"id":"22138","type":"BasicTicker"},{"attributes":{"overlay":{"id":"22150"}},"id":"22145","type":"LassoSelectTool"},{"attributes":{},"id":"22178","type":"BasicTickFormatter"},{"attributes":{},"id":"22180","type":"UnionRenderers"},{"attributes":{},"id":"22147","type":"SaveTool"},{"attributes":{"source":{"id":"22160"}},"id":"22165","type":"CDSView"},{"attributes":{},"id":"22146","type":"UndoTool"},{"attributes":{"coordinates":null,"formatter":{"id":"22178"},"group":null,"major_label_policy":{"id":"22179"},"ticker":{"id":"22134"}},"id":"22133","type":"LinearAxis"},{"attributes":{},"id":"22144","type":"WheelZoomTool"},{"attributes":{},"id":"22182","type":"UnionRenderers"},{"attributes":{"fill_alpha":0.1,"fill_color":"#ff0000","hatch_alpha":0.1,"line_alpha":0.1,"line_color":"#ff0000","x":{"field":"x"},"y":{"field":"y"}},"id":"22162","type":"Patch"},{"attributes":{"axis":{"id":"22133"},"coordinates":null,"group":null,"ticker":null},"id":"22136","type":"Grid"},{"attributes":{"axis":{"id":"22137"},"coordinates":null,"dimension":1,"group":null,"ticker":null},"id":"22140","type":"Grid"},{"attributes":{"fill_alpha":0.5,"fill_color":"#ff0000","line_alpha":0,"line_color":"#ff0000","x":{"field":"x"},"y":{"field":"y"}},"id":"22161","type":"Patch"},{"attributes":{"callback":null},"id":"22148","type":"HoverTool"},{"attributes":{"coordinates":null,"data_source":{"id":"22160"},"glyph":{"id":"22161"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"22163"},"nonselection_glyph":{"id":"22162"},"view":{"id":"22165"}},"id":"22164","type":"GlyphRenderer"},{"attributes":{"coordinates":null,"formatter":{"id":"22175"},"group":null,"major_label_policy":{"id":"22176"},"ticker":{"id":"22138"}},"id":"22137","type":"LinearAxis"},{"attributes":{"data":{"x":{"__ndarray__":"","dtype":"float64","order":"little","shape":[100]},"y":{"__ndarray__":"","dtype":"float64","order":"little","shape":[100]}},"selected":{"id":"22183"},"selection_policy":{"id":"22182"}},"id":"22166","type":"ColumnDataSource"},{"attributes":{},"id":"22175","type":"BasicTickFormatter"},{"attributes":{},"id":"22134","type":"BasicTicker"},{"attributes":{"overlay":{"id":"22149"}},"id":"22143","type":"BoxZoomTool"},{"attributes":{"bottom_units":"screen","coordinates":null,"fill_alpha":0.5,"fill_color":"lightgrey","group":null,"left_units":"screen","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"right_units":"screen","syncable":false,"top_units":"screen"},"id":"22149","type":"BoxAnnotation"},{"attributes":{"below":[{"id":"22133"}],"center":[{"id":"22136"},{"id":"22140"}],"height":500,"left":[{"id":"22137"}],"output_backend":"webgl","renderers":[{"id":"22164"},{"id":"22170"}],"title":{"id":"22172"},"toolbar":{"id":"22151"},"toolbar_location":"above","width":500,"x_range":{"id":"22125"},"x_scale":{"id":"22129"},"y_range":{"id":"22127"},"y_scale":{"id":"22131"}},"id":"22124","subtype":"Figure","type":"Plot"},{"attributes":{"line_width":3,"x":{"field":"x"},"y":{"field":"y"}},"id":"22167","type":"Line"},{"attributes":{},"id":"22176","type":"AllLabels"},{"attributes":{},"id":"22131","type":"LinearScale"},{"attributes":{"tools":[{"id":"22141"},{"id":"22142"},{"id":"22143"},{"id":"22144"},{"id":"22145"},{"id":"22146"},{"id":"22147"},{"id":"22148"}]},"id":"22151","type":"Toolbar"}],"root_ids":["22124"]},"title":"Bokeh Application","version":"2.4.1"}}';
                  const render_items = [{"docid":"7a0d7b42-5d9c-4cc5-823c-3f7ba11966e6","root_ids":["22124"],"roots":{"22124":"a2506096-24f6-4a32-9ed0-f46522568a89"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    let attempts = 0;
                    const timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        clearInterval(timer);
                        embed_document(root);
                      } else {
                        attempts++;
                        if (attempts > 100) {
                          clearInterval(timer);
                          console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        }
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
        
        
        }
      ];
    
      function run_inline_js() {
        
        for (let i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();