(function() {
  const fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      const force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      const element = document.getElementById("6662b539-5909-420c-8139-c76f340bac00");
        if (element == null) {
          console.warn("Bokeh: autoload.js configured with elementid '6662b539-5909-420c-8139-c76f340bac00' but no matching script tag was found.")
        }
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error(url) {
          console.error("failed to load " + url);
        }
    
        for (let i = 0; i < css_urls.length; i++) {
          const url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        for (let i = 0; i < js_urls.length; i++) {
          const url = js_urls[i];
          const element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.async = false;
          element.src = url;
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      
      const js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-2.4.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-gl-2.4.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.4.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.4.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-mathjax-2.4.1.min.js"];
      const css_urls = [];
      
    
      const inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          (function() {
            const fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  const docs_json = '{"d2d7ba23-9bbb-479d-b033-26ac6d5f1898":{"defs":[],"roots":{"references":[{"attributes":{"data":{},"selected":{"id":"17872"},"selection_policy":{"id":"17871"}},"id":"17852","type":"ColumnDataSource"},{"attributes":{"coordinates":null,"formatter":{"id":"17862"},"group":null,"major_label_policy":{"id":"17863"},"ticker":{"id":"17817"}},"id":"17816","type":"LinearAxis"},{"attributes":{"coordinates":null,"data_source":{"id":"17852"},"glyph":{"id":"17853"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"17855"},"nonselection_glyph":{"id":"17854"},"view":{"id":"17857"}},"id":"17856","type":"GlyphRenderer"},{"attributes":{"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"17853","type":"Line"},{"attributes":{},"id":"17867","type":"UnionRenderers"},{"attributes":{"axis":{"id":"17816"},"coordinates":null,"dimension":1,"group":null,"ticker":null},"id":"17819","type":"Grid"},{"attributes":{},"id":"17825","type":"UndoTool"},{"attributes":{"tools":[{"id":"17820"},{"id":"17821"},{"id":"17822"},{"id":"17823"},{"id":"17824"},{"id":"17825"},{"id":"17826"},{"id":"17827"}]},"id":"17830","type":"Toolbar"},{"attributes":{"toolbars":[{"id":"17830"}],"tools":[{"id":"17820"},{"id":"17821"},{"id":"17822"},{"id":"17823"},{"id":"17824"},{"id":"17825"},{"id":"17826"},{"id":"17827"}]},"id":"17877","type":"ProxyToolbar"},{"attributes":{},"id":"17866","type":"AllLabels"},{"attributes":{},"id":"17870","type":"Selection"},{"attributes":{"overlay":{"id":"17829"}},"id":"17824","type":"LassoSelectTool"},{"attributes":{"line_alpha":0.2,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"17842","type":"Line"},{"attributes":{"coordinates":null,"formatter":{"id":"17865"},"group":null,"major_label_policy":{"id":"17866"},"ticker":{"id":"17813"}},"id":"17812","type":"LinearAxis"},{"attributes":{"below":[{"id":"17812"}],"center":[{"id":"17815"},{"id":"17819"},{"id":"17845"}],"height":500,"left":[{"id":"17816"}],"output_backend":"webgl","renderers":[{"id":"17843"},{"id":"17850"},{"id":"17856"}],"title":{"id":"17858"},"toolbar":{"id":"17830"},"toolbar_location":null,"width":500,"x_range":{"id":"17804"},"x_scale":{"id":"17808"},"y_range":{"id":"17806"},"y_scale":{"id":"17810"}},"id":"17803","subtype":"Figure","type":"Plot"},{"attributes":{"coordinates":null,"data_source":{"id":"17839"},"glyph":{"id":"17840"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"17842"},"nonselection_glyph":{"id":"17841"},"view":{"id":"17844"}},"id":"17843","type":"GlyphRenderer"},{"attributes":{"line_alpha":0.1,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"17854","type":"Line"},{"attributes":{},"id":"17821","type":"PanTool"},{"attributes":{},"id":"17823","type":"WheelZoomTool"},{"attributes":{"children":[[{"id":"17803"},0,0]]},"id":"17876","type":"GridBox"},{"attributes":{"source":{"id":"17839"}},"id":"17844","type":"CDSView"},{"attributes":{"line_alpha":0.1,"line_color":"white","x":{"field":"x"},"y":{"field":"y"}},"id":"17848","type":"Line"},{"attributes":{},"id":"17817","type":"BasicTicker"},{"attributes":{"coordinates":null,"data_source":{"id":"17846"},"glyph":{"id":"17847"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"17849"},"nonselection_glyph":{"id":"17848"},"view":{"id":"17851"}},"id":"17850","type":"GlyphRenderer"},{"attributes":{},"id":"17865","type":"BasicTickFormatter"},{"attributes":{"source":{"id":"17846"}},"id":"17851","type":"CDSView"},{"attributes":{},"id":"17863","type":"AllLabels"},{"attributes":{"data":{"x":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]},"y":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]}},"selected":{"id":"17868"},"selection_policy":{"id":"17867"}},"id":"17839","type":"ColumnDataSource"},{"attributes":{},"id":"17804","type":"DataRange1d"},{"attributes":{},"id":"17868","type":"Selection"},{"attributes":{"axis":{"id":"17812"},"coordinates":null,"group":null,"ticker":null},"id":"17815","type":"Grid"},{"attributes":{"line_alpha":0.1,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"17841","type":"Line"},{"attributes":{},"id":"17862","type":"BasicTickFormatter"},{"attributes":{"coordinates":null,"group":null,"text":"y / y","text_font_size":"15pt"},"id":"17858","type":"Title"},{"attributes":{},"id":"17810","type":"LinearScale"},{"attributes":{"line_alpha":0.2,"line_color":"white","x":{"field":"x"},"y":{"field":"y"}},"id":"17849","type":"Line"},{"attributes":{"children":[{"id":"17878"},{"id":"17876"}]},"id":"17879","type":"Column"},{"attributes":{},"id":"17813","type":"BasicTicker"},{"attributes":{"bottom_units":"screen","coordinates":null,"fill_alpha":0.5,"fill_color":"lightgrey","group":null,"left_units":"screen","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"right_units":"screen","syncable":false,"top_units":"screen"},"id":"17828","type":"BoxAnnotation"},{"attributes":{},"id":"17808","type":"LinearScale"},{"attributes":{},"id":"17871","type":"UnionRenderers"},{"attributes":{"callback":null},"id":"17827","type":"HoverTool"},{"attributes":{"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"17840","type":"Line"},{"attributes":{"line_alpha":0.2,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"17855","type":"Line"},{"attributes":{"coordinates":null,"fill_alpha":0.5,"fill_color":"lightgrey","group":null,"level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"syncable":false,"xs_units":"screen","ys_units":"screen"},"id":"17829","type":"PolyAnnotation"},{"attributes":{},"id":"17872","type":"Selection"},{"attributes":{"overlay":{"id":"17828"}},"id":"17822","type":"BoxZoomTool"},{"attributes":{"source":{"id":"17852"}},"id":"17857","type":"CDSView"},{"attributes":{},"id":"17869","type":"UnionRenderers"},{"attributes":{},"id":"17806","type":"DataRange1d"},{"attributes":{"bottom":1.459677659268402,"coordinates":null,"fill_alpha":0.1,"fill_color":"#2a2eec","group":null,"top":0.6850827603275131},"id":"17845","type":"BoxAnnotation"},{"attributes":{},"id":"17820","type":"ResetTool"},{"attributes":{"data":{"x":[0,1],"y":[1,1]},"selected":{"id":"17870"},"selection_policy":{"id":"17869"}},"id":"17846","type":"ColumnDataSource"},{"attributes":{"toolbar":{"id":"17877"},"toolbar_location":"above"},"id":"17878","type":"ToolbarBox"},{"attributes":{"line_color":"white","x":{"field":"x"},"y":{"field":"y"}},"id":"17847","type":"Line"},{"attributes":{},"id":"17826","type":"SaveTool"}],"root_ids":["17879"]},"title":"Bokeh Application","version":"2.4.1"}}';
                  const render_items = [{"docid":"d2d7ba23-9bbb-479d-b033-26ac6d5f1898","root_ids":["17879"],"roots":{"17879":"6662b539-5909-420c-8139-c76f340bac00"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    let attempts = 0;
                    const timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        clearInterval(timer);
                        embed_document(root);
                      } else {
                        attempts++;
                        if (attempts > 100) {
                          clearInterval(timer);
                          console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        }
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
        
        
        }
      ];
    
      function run_inline_js() {
        
        for (let i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();